...
{
    [super viewDidLoad];
    [self highlightCommandNameInInfo];
...
/**
 * Searches for the macDownMod shell utility and invokes foundShellUtilityAtURL: if found.
 */
- (void)lookForShellUtility
{
    ...
    MPDetectHomebrewPrefixWithCompletionhandler(^(NSString *output) {
        NSString *commandPath = MPCommandInstallationPath;
        if (output)
        {
            ...
            commandPath =
                [prefix stringByAppendingPathComponent:@"bin/macDownMod"];
        }

        if ([[NSFileManager defaultManager] fileExistsAtPath:commandPath])
            weakSelf.shellUtilityURL = [NSURL fileURLWithPath:commandPath];
    });
}

- (void)installShellUtility
{
    // URL for the shell utility in the app bundle.
    NSURL *sharedSupportURL = [NSBundle mainBundle].sharedSupportURL;
    NSString *utilityBundlePath =
        [sharedSupportURL URLByAppendingPathComponent:@"bin/macDownMod"].path;
...
/**
 * Highlights the command name in the info text.
 */
- (void)highlightCommandNameInInfo
{
