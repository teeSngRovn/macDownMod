...
void MPCollectPipedContentURLForApp(NSURL *url) {
...
void MPCollectForApp(NSOrderedSet<NSURL *> *urls)
{
    NSUserDefaults *defaults =
        [[NSUserDefaults alloc] initWithSuiteNamed:kMPApplicationSuiteName];
...
/**
 * Data piped to macDownMod through stdin.
 * 
...
int main(int argc, const char * argv[])
{
    @autoreleasepool
    {
        ...
        if (dataFromPipe) {
            // Store piped content in a temporary file which will be read by macDownMod on launch
            ...
            if (writeError == nil) {
                MPCollectPipedContentURLForApp(fileURL);
            }
        }

        // Treat all arguments as file names to open. Convert them to absolute
        // paths and store them (as an array) in macDownMod's user defaults to
        // be opened later.
